...
const Planet = ({ planet }) => {
    ...
    const handleModalOpen = () => {
        const modal = document.getElementById(planet._id);
        modal.showModal()
        if (modal) {
            modal.showModal(); 
        }
...
<div className="collapse bg-slate-950 border border-solid border-transparent hover:border-orange-500">
            <div className='collapse text-slate-50 hover:border-orange-500'>
                ...
                <dialog id={planet._id} className="modal">
                    {planet === null ? (
                        ...
                        <div className="modal-box p-4 rounded-lg shadow-md flex flex-col items-center">
                            <p className="text-gray-900 dark:text-slate-50"><span className="font-semibold text-gray-900 dark:text-slate-50"><FontAwesomeIcon icon={faTemperatureLow} className="mr-2" />Climate:</span> {planet.climate}</p>
                            <p className="text-gray-900 dark:text-slate-50"><span className="font-semibold text-gray-900 dark:text-slate-50"><FontAwesomeIcon icon={faMountain} className="mr-2" />Terrain:</span> {planet.terrain}</p>
                            <p className="text-gray-900 dark:text-slate-50"><span className="font-semibold text-gray-900 dark:text-slate-50"><FontAwesomeIcon icon={faUser} className="mr-2" />Population:</span> {planet.population}</p>
