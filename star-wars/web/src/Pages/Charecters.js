...
export const Charecters = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setPage(1);
    setSearch(true);
    fetch(`http://127.0.0.1:4000/characters/?name=${name}&pageSize=12`)
      .then(response => {
  ...
  React.useEffect(() => {
    async function fetchPeople() {
      try {
        ...
        const response = await fetch(`http://127.0.0.1:4000/characters/?page=${page}&pageSize=12`);
  ...
  return (
    <>
      ...
      <main>
        ...
        <div className="flex justify-center items-center h-3/5 hero-overlay pb-3">
        ...
        <div className="flex justify-center items-center hero-overlay">
          <div className="container mx-auto px-4 mt-100vh h-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {people.map(person => (
                <Character key={person._id} person={person} />  
              ))}
            </div>  
          </div>
        </div>
